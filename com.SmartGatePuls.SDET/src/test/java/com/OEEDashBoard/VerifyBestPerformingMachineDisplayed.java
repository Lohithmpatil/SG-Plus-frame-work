...
public class VerifyBestPerformingMachineDisplayed extends BaseClass {

	@Test
	public void VerifyBestPerformingMachineDisplayed() throws Throwable {
		...
		we.waitAndClick(oe.getBestPerformingMachineOEE());

		// Locate all OEE values from Best Performing Machines table
		List<WebElement> oeeElements = driver.findElements(
				By.xpath("//div[contains(text(),'BEST PERFORMING MACHINES')]/following::table[1]//tbody/tr/td[2]"));
		...
		if (actualValues.equals(expectedValues)) {
			System.out.println("PASS : Best Performing Machines are sorted correctly");
		} else {
			System.out.println("FAIL : Best Performing Machines are NOT sorted correctly");
