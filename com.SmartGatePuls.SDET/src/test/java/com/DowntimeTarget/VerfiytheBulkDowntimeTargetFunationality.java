...
public class VerfiytheBulkDowntimeTargetFunationality extends BaseClass{
	

	@Test
	public void VerfiytheBulkDowntimeTargetFunationality() throws Throwable {
		...
		WebElement successMsg = wait1.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(
				"//div[contains(@class,'Toastify__toast-body') or contains(text(),'10 Loss target(s) updated successfully')]")));

		String actualMessage = successMsg.getText().trim();

		System.out.println("Success Message: " + actualMessage);

		Assert.assertEquals(actualMessage, "10 Loss target(s) updated successfully",
