...
public class VeriftyheExportFeatureBySelectingEquipement extends BaseClass{
	@Test
	public void VeriftyheExportFeatureBySelectingEquipement() throws Throwable {
		...
		we.select(cd.getXaxisDropdown(), "By Equipment");

		we.waitAndClick(cd.getExportButton());

		WebElement exportBtn = cd.getExportButton();

		Assert.assertTrue(exportBtn.isDisplayed(), "Export button is not displayed");
		Assert.assertTrue(exportBtn.isEnabled(), "Export button is disabled");
		...
		Assert.assertTrue(afterFiles.length > beforeFiles.length, "Export file was not downloaded");

		Assert.assertTrue(afterFiles.length > beforeFiles.length, "Export file download failed.");

		System.out.println("Selecting By Equipement Export functionality is working successfully.");
