...
public class VerifytheWeightedAvgCTinMinute extends BaseClass{
	
	@Test
	public void verifytheWeightedAvgCTMinute() throws Throwable {
		...
		we.waitAndClick(cd.getUnitdropdown());
		
		
		we.select(cd.getUnitdropdown(), "Minutes");
		
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));

		WebElement weightedAvgElement = wait.until(
		        ExpectedConditions.visibilityOfElementLocated(
		                By.xpath("//th[contains(text(),'Weighted Avg Cycle Time')]/following-sibling::th[1]")));

		String weightedAvg = weightedAvgElement.getText();

		System.out.println("Weighted Avg Cycle Time in Minute : " + weightedAvg);
}
