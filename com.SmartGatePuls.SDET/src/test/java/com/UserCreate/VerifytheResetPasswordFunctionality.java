...
public class VerifytheResetPasswordFunctionality extends BaseClass {

	@Test
	public void VerifytheResetPasswordFunctionality() throws Throwable {
		...
		we.waitAndClick(us.getResetPasswordButton());

		we.waitAndClick(us.getEnterNewPassword());

		us.getEnterNewPassword().sendKeys(password);

		we.waitAndClick(us.getOKNewPasswordButton());
		...
		// Verify the message
		Assert.assertTrue(actualMsg.contains("User password change/reset successful"),
				"User password change/reset  failed. Actual message: " + actualMsg);
