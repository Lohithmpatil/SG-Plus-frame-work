...
public class VerifytheUserChangeStatusCancelFuctionality extends BaseClass{
	@Test
	public void VerifytheUserChangeStatusCancelFuctionality() throws Throwable {
		...
		we.waitAndClick(us.getNOStatusChangeButton());
		
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));

		boolean popupClosed = wait.until(
		    ExpectedConditions.invisibilityOfElementLocated(
		        By.xpath("//h4[contains(text(),'Reset Password')]")));

		Assert.assertTrue(popupClosed,
		        "Change User Status popup did not close after clicking Cancel");
